...
export default function home({navigation}) {
  ...
  function playstopvideo() {
    return (
      <View style={{ justifyContent: 'center', textAlign: 'center', alignItems: 'center' }} >
        <TouchableOpacity onPress={ recording ? stopVideo : recordVideo  } style={styles.buttonbackground}   >
  ...
  async function submit() {
    ...
    console.log('id',id)
    const emailid=await AsyncStorage.getItem('email-id')
    ...
    await axios.post('https://particle-ae921-default-rtdb.firebaseio.com/job.json', {
      ...
      recruiterId:navigation.getParam('recruiterId'),
      video: link,
    })
      .then(res => {
        console.log(res)
